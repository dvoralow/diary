...
raw_data <- read.csv("Z:/Fisherman diary/Full excel/2009-2010/2009-2010.csv")
...
ind<-c(0)
kg<-c(0)
species_vector<-c(20:62,65:72,74:82)
...
for(i in nets){
  ...
  net_template<-cbind(raw_data[1:19][i,],raw_data$season[i],raw_data$Very_weak[i],raw_data$Missing[i],raw_data$Comments[i],species,size,boxes,ind,kg) # net data might have more than 1 rows 
  colnames(net_template)[20]<-"very_weak"
      ...
      if(location_of_first_[[1]][1]==-1){ # species cell have just one number
        ...
        species_data$boxes<-as.numeric(as.character(raw_data[i,j]))
          ...
          for(letter in c("I","B","M","S","L","K")){ # which letters are in the cell
            location_of_letter <- (gregexpr(pattern =letter,raw_data[i,j]))
...
total_boxes_per_net<-rep(0,max(data$net))
draging_time<-rep(0,max(data$net))
...
effort<-data.frame(raw_data$Net,effort_per_hour_per_net,raw_data$season,dayOrNight,raw_data$Fishermen,depth,total_boxes_per_net,raw_data$draging.time)
colnames(effort)<-c("net","effort","season","dayOrNight","fisherMan","depth","total_boxes_per_net","dragingTime")
