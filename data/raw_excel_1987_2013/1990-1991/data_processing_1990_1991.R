...
raw_data <- read.csv("Z:/Fisherman diary/Full excel/1990-1991/1990-1991_enav.csv")
...
for(i in nets){
  ...
  #raw_data$Very_weak[i]
  net_template<-cbind(raw_data[1:16][i,],raw_data$season[i],raw_data$Missing[i],raw_data$Comments[i],raw_data$draging.time[i],species,size,boxes,ind,kg) # net data might have more than 1 rows 
  colnames(net_template)[17]<-"season"
  #colnames(net_template)[21]<-"very_weak"
  colnames(net_template)[18]<-"missing"
  colnames(net_template)[19]<-"comments"
  colnames(net_template)[20]<-"draging.time"
