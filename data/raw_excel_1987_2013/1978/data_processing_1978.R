...
raw_data <- read.csv("Z:/Fisherman diary/Full excel/1978/1978.csv")
...
ind<-c(NA)
kg<-c(NA)
species_vector<-c(22:57,60:67,69:78)
...
for(i in nets){
  ...
  net_template<-cbind(raw_data[1:21][i,],raw_data$season[i],raw_data$Very_weak[i],raw_data$Missing[i],raw_data$Comments[i],raw_data$draging.time[i],species,size,boxes,ind,kg) # net data might have more than 1 rows 
  colnames(net_template)[22]<-"season"
  colnames(net_template)[23]<-"very_weak"
  colnames(net_template)[24]<-"missing"
  colnames(net_template)[25]<-"comments"
  colnames(net_template)[26]<-"draging.time"
