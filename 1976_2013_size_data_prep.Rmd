...
write.csv(kg_data, "pross size 1976_2013/kg_data_sizes.CSV")
...
```{r}
 kg_data = read.csv("pross size 1976_2013/kg_data_sizes.CSV")
...
write.csv(new_data,file = "pross size 1976_2013/kg_data_sizes.csv")
...
empty_row = data.frame(matrix(NA, nrow = 31, ncol = 3)) 
empty_row <- setNames(empty_row, c("Sp_code","Common_name","Sceintific_name"))

sceintific_names = rbind(sceintific_names, empty_row)

sceintific_names = cbind(sceintific_names, species_vector)
#I export that file and order in the excel file the the namesm, creating new file, "species to scientific code_for raw 1976_2013 data" with the whole names exept some rare and unknown common names.
...
kg_data = kg_data %>% filter(species != "Jelly_Fish") %>% 
      filter(species != "box_only") %>%  filter(species != "Night_drag") %>% 
    filter(species != "Tuna") %>% filter(species != "Tuna") %>%
    filter(species != "Botz") %>% filter(species != "yasmin")  %>%
    filter(species != "Soltan_mesudar")%>% filter(species != "san_fedro")%>% 
    filter(species != "raskas") %>% filter(species != "Raskasim")  %>%
    filter(species != "labat")%>%  filter(species != "Hazaim")%>%
    filter(species != "gagim") %>% filter(species != "Krisal") %>%
    filter(species != "Cristal")%>% filter(species != "Gambari_adom")%>%
    filter(species != "Vest")%>% filter(species != "vest")%>% filter(species != "Adom")

#update the species vector
  species_vector = unique(kg_data$species)
species_vector= sort(species_vector)

x =kg_data %>%  mutate(species = replace(kg_data$species, "Musar","mmm"))



   
 #uploud an update of the scintific name - data with common names, origin, code, and scientific names - I put inside also duplicates code with to differences common names.
code_sceintific_names= read.csv(file="pross size 1976_2013/species to scientific code_for raw 1976_2013 data.csv", stringsAsFactors = F)
...
{
  for (j in 1:length(code_sceintific_names$Common_name))
  {
    if (kg_data$species[i]==code_sceintific_names$Common_name[j])
{
  ...
  write.csv(kg_data, "pross size 1976_2013/kg_data_code.CSV")
