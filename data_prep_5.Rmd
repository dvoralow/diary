...
#replace the "kg" in the colnames of the species
 ...
 #order the data
  ...
  write.csv(wide_data, "prosseced data/wide_new_data.CSV")
  wide_data = read.csv("prosseced data/wide_new_data.CSV")
...
```{r}
#check the function:
Lon=loc_data$Lon_shore[1]
Lat = loc_data$Lat_shore[1]
Perpendicular = loc_data$Perpendicular[1]
target_depth = loc_data$Depth_m[1]
...
location_depth_based=function(Lon,Lat,Perpendicular,target_depth){
  ...
  #create a data frame with the depath in each Lan-Lot of the specific Perpendicular
  for (d in distances){
  ...
  #if there is no locathions and depth (because the Perpendicular is NA I think) return NA in the Lon_final Lat_final Depth_diff
  if (nrow(loc_dt[complete.cases(loc_dt),])==0){
   ...
   #find the depth is the most similar to the input depth. [min of the differences] 
      ...
      print(min(abs(loc_dt$depth-(-1*target_depth)),na.rm = T))
        ...
        target_location=some(target_location,n=1)
