...
required if dont working from project.
make a variable called "thesis_directory" with my location of the documents.
this is a good solution if you work on google drive in different computers
```{r}
...
```{r}
#upload the data
raw_data <- read.csv(file=paste(thesis_directory,"all_data_2013_2018.csv",sep = "/"), stringsAsFactors=FALSE)
...
write.csv(data,file = paste(thesis_directory,"R/prosseced data/new_data_prosseced_raw.csv",sep = "/"))

data = read.csv(file =paste( thesis_directory,"R/prosseced data/new_data_prosseced_raw.csv",sep = "/"))
...
write.csv(new_data,file = paste(thesis_directory,"R/prosseced data/new_data_one_kg_col.csv",sep = "/"))
new_data = read.csv(file = paste(thesis_directory,"R/prosseced data/new_data_one_kg_col.csv",sep = "/"))


```

function for putting zeros where species is not found - create raws withe zero
for the species isn't written.

Input:1. Data frame with any species in any net in seperate raw.
      Contain the columns: "Net_index", "species","Kg"
      
      2. Vector with the species names apears in the data$species
      
Output: Data frame with the columns:"net_index", "sp", "kg"
        contain seperate raw for any species in any net - species didn't apear in 
        a net of the input [species didn't catched in a specific net]
        is wrriten with amount of 0 and unit of "zero filled"

```{r}

data_add_zero_F=function(in_data,sp_list){
  zero_data=data.frame()
  #run the loop for any species
  for (sp in sp_list){
    print(sp)
    net_sp=data.frame()
    #run the loop for any net
    for(net in unique(in_data$Net_index)){
      #print(net)
      #for(net in c("1_21_9_2009")){
      net_data=in_data[in_data$Net_index==net,]
      #if the species is in the net, copy it's information
      if (sp %in% net_data$species){
        sp_in_net=subset(net_data,net_data$species==sp)
        net_c=data.frame(net_index=as.character(sp_in_net$Net_index),sp=as.character(sp_in_net$species),kg=as.numeric(sp_in_net$Kg))
#Itay has also size column. I hasn't, at least for now. 
        #,size=as.character(sp_in_net$size)        
#print(net_c)
        #if the species doesn't exist in the net put it as 0
      }else{
        net_c=data.frame(net_index=net,sp=sp,kg=0) 
        #,size=NA
      }
      net_sp=rbind(net_sp,net_c)
    }
    zero_data=rbind(zero_data,net_sp)
  }
  return(zero_data)
}
...
call to the data_add_zero function, save and edit

```{r}
zero_data=data_add_zero_F(new_data,species_vector)

write.csv(zero_data,file = paste(thesis_directory,"R/prosseced data/temporary_zero_data.csv",sep = "/"))
zero_data=read.csv(paste(thesis_directory,"R/prosseced data/temporary_zero_data.csv",sep = "/"))
...
!!!------ It arese the duplicate of the Lokus and Musar I created before. need to sum them instead -----!!!!!

```{r}
zero_data$net_index=as.factor(zero_data$net_index)
zero_data$sp=as.character(zero_data$sp)
wide_data <- reshape(zero_data,timevar = "sp",idvar = c("net_index"),direction = "wide")
...
sceintific_names=read.csv(paste(thesis_directory,"species_scientific_name.csv",sep = "/"))
...
names_vector =   colnames(dplyr :: select(wide_data,kg.Anshubi:kg.Aras))
  ...
  names_vector[i] = gsub("kg.", "", names_vector[i] )  
   ...
   write.csv(names_vector,file = paste(thesis_directory,"R/prosseced data/names_vector.csv",sep = "/"))
...
code_sceintific_names= read.csv(file=paste(thesis_directory,"species_scientific_name_new_data.csv",sep = "/"), stringsAsFactors=FALSE)
...
setwd("C:/Users/Admin/Google Drive/second degree/thesis/data")
...
#import data frame with shore loctions
location_data <- read.csv(paste(thesis_directory,"locations_lat_on_shore.csv",sep = "/"))
colnames(location_data)=c("site","lat_shore","lon_shore","perpendicular")

#merge the data
proccessed_data=merge(proccessed_data,location_data,by.x="new_site",by.y="site")

#subset to unique nets using the 'net_index' column
data_unique_net=proccessed_data[!duplicated(proccessed_data$net_index),]
#phatom to meters
data_unique_net$depth_meter=1.8*data_unique_net$Depth
#nautical miles to Km
data_unique_net$distance_from_shore_m=1.8*data_unique_net$Distance_to_Shore
...
#for nets with depth
data_unique_net$depth_specifyed=rep(NA,nrow(data_unique_net))
data_unique_net[!is.na(data_unique_net$depth_meter),"depth_specifyed"]=T
#for nets with distance
data_unique_net$distance_specifyed=rep(NA,nrow(data_unique_net))
data_unique_net[!is.na(data_unique_net$distance_from_shore_m),"distance_specifyed"]=T
#for nets with starting position
data_unique_net$location_specifyed=rep(NA,nrow(data_unique_net))
data_unique_net[!is.na(data_unique_net$new_site),"location_specifyed"]=T
...
data_unique_net$Lon_final=rep(NA,nrow(data_unique_net))
data_unique_net$Lat_final=rep(NA,nrow(data_unique_net))
data_unique_net$depth_diff=rep(NA,nrow(data_unique_net))
#nrow(data_unique_net)
nrow(data_unique_net)
for (r in 1:nrow(data_unique_net)){
  print(r)
  row_c=data_unique_net[r,]
  if (!is.na(row_c$lat_shore) & !is.na(row_c$depth_meter)){
    final_loc=location_depth_based(data_unique_net[r,"lon_shore"],data_unique_net[r,"lat_shore"],data_unique_net[r,"perpendicular"],data_unique_net[r,"depth_meter"])
    data_unique_net[r,"Lon_final"]=final_loc[1,"lon_final"]
    data_unique_net[r,"Lat_final"]=final_loc[1,"lat_final"]
    data_unique_net[r,"depth_diff"]=final_loc[1,"depth_diff"]
  }else{}
}
...
for (r in 1:nrow(data_unique_net)){
  print(r)
  row_c=data_unique_net[r,]
  if (!is.na(row_c$lat_shore) & is.na(row_c$depth_meter) & !is.na(row_c$distance_from_shore_m)){
    final_loc=location_distance_based(data_unique_net[r,"lon_shore"],data_unique_net[r,"lat_shore"],data_unique_net[r,"perpendicular"],data_unique_net[r,"distance_from_shore_m"])
    data_unique_net[r,"Lon_final"]=final_loc[1,"lon_final"]
    data_unique_net[r,"Lat_final"]=final_loc[1,"lat_final"]
  }else{}
}
...
data_unique_net$loc_DEPTH=extract(med_rast1, data_unique_net[,c("Lon_final","Lat_final")])
data_unique_net$loc_DEPTH=abs(data_unique_net$loc_DEPTH)
##Test if location based depth are good
#summary
summary(data_unique_net$loc_DEPTH)
#compare to nets with specified depth
depth_specified=subset(data_unique_net,depth_specifyed==T)
depth_specified$depth_diff=depth_specified$loc_DEPTH-depth_specified$depth_meter
depth_check=data.frame(observed_depth=depth_specified$depth_meter,location_depth=depth_specified$loc_DEPTH,depth_diff=depth_specified$depth_meter-depth_specified$loc_DEPTH)
summary(depth_check$depth_diff)
plot(depth_specified$depth_meter,depth_specified$loc_DEPTH)
...
```{r}
#upload the data
proccessed_data <- read.csv(file=paste(thesis_directory,"all_data_2013_2018.csv",sep = "/"), stringsAsFactors=FALSE)
#import data frame with shore loctions
location_data <- read.csv(file = paste(thesis_directory,"locations_lat_on_shore.csv",sep = "/"), stringsAsFactors = FALSE)
